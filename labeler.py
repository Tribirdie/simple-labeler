...
def incr_pos():
    ...
    if current_file != amount_of_entries:
        current_file += 1
    print(current_file)
...
def draw_square():
    global file_entries
    print(file_entries)
    matrix = cv2.imread(file_entries[0])
    cv2.imshow("gay window", matrix)
    cv2.waitKey(0)
...
def main():
    ...
    ttk.Button(gui, text="Up", command=incr_pos).place(height=75, width=275, x=405, y=330)
    ttk.Button(gui, text="Down", command=lower_pos).place(height=70, width=275, x=405, y=410)
